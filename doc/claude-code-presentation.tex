...
\begin{frame}{Real Example: We Just Did This}
  ...
  \begin{columns}
    ...
    \begin{itemize}
      \item tsconfig/ESLint/Prettier: 2 min
      \item Neovim TypeScript support: 1 min
      \item Bootstrap script: 3 min
      \item Git hooks: 2 min
...
\begin{frame}{TypeScript Framework Support}
  ...
  \begin{itemize}
    ...
    \item ESLint + Prettier pre-configured
    \item Git hooks (pre-commit format, pre-push type-check)
...
\begin{frame}{Git Hooks: Quality Gates on Autopilot}
  ...
  \begin{columns}
    ...
    \begin{itemize}
      ...
      \item Pre-push: tsc + ESLint
      \item Fix: \texttt{npm run format}
      \item Type errors blocked
    \end{itemize}
...
\begin{frame}{Productivity Gains: By The Numbers}
  \begin{table}
    ...
    \begin{tabular}{lcc}
      ...
      TSConfig + ESLint + Prettier & 15-25 min & 0 min (automatic) \\
...
\begin{frame}{Live Demo: The Three Quality Gates}
...
const Counter = (): JSX.Element => {  // <-- Explicit!
  ...
  \vspace{0.5em}
  \centering
  \textbf{All three gates passed — code is ready to push!}
\end{frame}
